...
function getFiles(dir) {
   return fs
      .readdirSync(dir)
      .filter((f) => f.endsWith(".svelte") || fs.lstatSync(`${dir}/${f}`).isDirectory())
      .map((p) => (fs.lstatSync(`${dir}/${p}`).isDirectory() ? getFiles(`${dir}/${p}`) : `${dir}/${p}`))
      .flat();
}
...
function createRoutes() {
   ...
   const findRoute = (dir) => {
      return fs
         .readdirSync(dir)
         .filter((f) => f.match(/[A-Z]\w+.svelte/) || fs.lstatSync(`${dir}/${f}`).isDirectory())
         .map((p) => (fs.lstatSync(`${dir}/${p}`).isDirectory() ? findRoute(`${dir}/${p}`) : `${dir}/${p}`))
         .flat()
         .sort();
   };
   ...
   files.map((x) => {
      ...
      let path = x.toLowerCase().replace(pages, "").replace(".svelte", "").split("/").join("/");
      if (cmp === "Index") path = "/";
...
function cmpIndex(filepath) {
   ...
   setTimeout(() => {
      ...
      files = files.filter((x) => x.endsWith(".svelte")).map((x) => x.split("/").splice(2).join("/"));
      let content = "";
      files.map((file) => {
...
function paging(dir) {
   ...
   dir = dir.replace(/\\/g, "/");
   if (dir === pages) return;
   writeFile(dir + "/+home.svelte", "<script>\n</script>\n\n<article>\n\t<h1>Home</h1>\n</article>", 1);
   writeFile(dir + "/pages.js", "export { default as home } from './+home.svelte';", 1);
   ...
   setTimeout(() => {
      ...
      fnames.map((fname) => {
         if (fname.includes("+") && fname.endsWith(".svelte")) {
            let cmp = fname.replace(".svelte", "").replace("+", "").toLowerCase();
